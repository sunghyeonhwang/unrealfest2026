...
function ufs_render_track_view($daySessions) {
    foreach (ufs_s_sched_slots($daySessions) as $time) {
        ...
        foreach ($inSlot as $s) {
            ...
            echo '<a href="session_s.php?id='.e($s['id']).'" data-sched-card data-track="'.e($s['track']).'" data-level="'.e($s['level']).'" data-topics="'.e($topics).'" class="block p-6 hover:bg-[#0e0f14] transition-colors'.($isKey ? ' bg-[rgba(0,193,213,0.03)]' : '').'">';
...
function ufs_render_grid_view($daySessions, $day) {
    ...
    foreach (ufs_s_sched_slots($daySessions) as $time) {
        ...
        foreach ($keys as $k) {
            ...
            echo '<tr class="border-b border-[#27272a]"><td class="p-3 text-sm font-bold text-white align-top sticky left-0 bg-[#09090b] z-10">'.e($k['time']).'</td>';
            echo '<td colspan="4" class="p-2"><a href="session_s.php?id='.e($k['id']).'" data-grid-cell data-track="키노트" data-level="'.e($k['level']).'" data-topics="'.e(implode(' ', ufs_session_topics($k))).'" class="block bg-[#00C1D5] hover:bg-[#00b0c2] p-6 rounded-[6px] transition-all relative overflow-hidden">';
            ...
            foreach ($gridTracks as $tr) {
                ...
                if ($cells) {
                    ...
                    foreach ($cells as $cell) {
                        ...
                        echo '<a href="session_s.php?id='.e($cell['id']).'" data-grid-cell data-track="'.e($cell['track']).'" data-level="'.e($cell['level']).'" data-topics="'.e($gtopics).'" class="block bg-[#0e0f14] p-5 hover:bg-[#111115] transition-colors transition-opacity flex-grow'.$minh.' flex flex-col gap-2">';
