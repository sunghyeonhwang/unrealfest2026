 ...
 *
 * ───────── 얼리버드 연장(전체 세션 공개 기념) 자동 전환 ─────────
 * 2026-07-14 00:00:00 KST 부터 "전체 세션 공개 기념 얼리버드 연장"(마감 7/27) 상태로 자동 전환.
 *   - ufs_extend_armed() : 자정 자동전환 스위치. 승인 후 true 로 배포. (false 면 프리뷰 파라미터로만 미리보기)
 *   - ?ufs_preview=KEY   : 라이브에 영향 없이 "연장 후 화면"을 실제 URL로 미리보기(쿠키로 이동 유지, no-store/noindex).
 *   - ufs_extended()     : 연장 상태 여부(프리뷰이거나, armed && 자정 경과).
 * 자정 전 일반 방문자에게는 어떤 흔적도 노출되지 않는다(마케팅 필수 요건). CF는 이 HTML을 캐시하지 않음(DYNAMIC). */
if (!function_exists('ufs_extend_armed')) {
function ufs_extend_armed() { return false; }                         // ★승인 후 true 로 재배포 → 자정 자동전환 활성
}
if (!function_exists('ufs_extend_start_ts')) {
function ufs_extend_start_ts() { return strtotime('2026-07-14 00:00:00 +0900'); }  // 전환 시각(KST 고정)
}
...
function ufs_extended() {                                             // 연장 상태(프리뷰 or 자정 자동전환)
    if (ufs_is_preview()) return true;
    return ufs_extend_armed() && (time() >= ufs_extend_start_ts());
}
}
