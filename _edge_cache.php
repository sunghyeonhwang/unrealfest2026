/* 엣지 TTL 을 5분→1시간으로 올린 이유(2026-08-17 측정)
 *   원본이 불안정해 '실제로 원본에 다녀와야 하는' 요청의 상당수가 504 로 죽는다.
 *   index.php 기준 hit 0% / expired 0% / miss 74% 가 504 — 즉 504 는 캐시가 비었을 때만 난다.
 *   그런데 5분 TTL 로는 적중률이 13.3% 밖에 안 됐다(콜로별로 방문 간격이 TTL 보다 길어서).
 *   TTL 을 올려 원본에 다녀오는 횟수 자체를 줄이는 것이 504 를 줄이는 가장 직접적인 방법이다.
 *   신선도는 관리자 저장 시 자동 퍼지(_cf_purge.php)로 보장한다.
