...
function ufs_directsend_post($postvars) {
...
function ufs_sms_call_date($product_code) {
    if ($product_code === 'NORMAL_20') { return '8월 20일(목) 오전 9시'; }
    if ($product_code === 'NORMAL_21') { return '8월 21일(금) 오전 9시'; }
    return '8월 20일(목)~21일(금) 오전 9시'; // 양일권 등 기본
}
}
...
function ufs_sms_product_label($product_code) {
    if ($product_code === 'NORMAL_20') { return 'Day 1 단일권 (8.20)'; }
    if ($product_code === 'NORMAL_21') { return 'Day 2 단일권 (8.21)'; }
    return '양일권 (8.20~21)';
}
}
...
function ufs_send_qr_mms($name, $phone, $apply_no, $product_code) {
    ...
    $postvars = '{'
        ...
        . ', "attaches":' . $attaches
        . '}';
    return ufs_directsend_post($postvars);
}
}

/* 텍스트 SMS — 온라인 등록완료 / 가상계좌 입금안내 등 (첨부 없음) */
if (!function_exists('ufs_send_text_sms')) {
function ufs_send_text_sms($name, $phone, $title, $message) {
    $phone = preg_replace('/[^0-9]/', '', $phone);
    if ($phone === '') { return false; }
...
function ufs_send_online_sms($name, $phone) {
    $title = "<언리얼 페스트 서울 2026> 온라인 등록 확인";
    $message = "<언리얼 페스트 서울 2026> 온라인 등록이 완료되었습니다.\n\n"
             . "행사 당일 첫 세션 30분 전, 등록하신 이메일과 문자로 시청 링크가 발송됩니다.\n\n"
             . "일시: 8월 20일(목)~21일(금)\n\n"
             . "자세한 내용은 홈페이지를 참고하세요.\n"
             . "https://epiclounge.co.kr/unrealfest2026/\n\n"
             . "- 언리얼 페스트 사무국";
    return ufs_send_text_sms($name, $phone, $title, $message);
}
}
...
function ufs_send_vbank_sms($name, $phone, $bank_num, $amount, $product_code) {
    $title = "<언리얼 페스트 서울 2026> 가상계좌 입금 안내";
