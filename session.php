...
// 상세페이지 전용 시간 표기 override (스케줄 그리드 슬롯 시간은 그대로 유지) — Atlas 25분 분할 세션 + 개별 강연시간 정정
$ufs_detail_time = array('d2-t4-s2a' => '11:30~11:55', 'd2-t4-s2b' => '11:55~12:20', 'd1-t4-s1' => '11:30~12:00');
...
$panel_intro = array(
    ...
    array('company'=>'트라이펄 게임즈', 'name'=>'정만손', 'bio'=>'트라이펄 게임즈의 CEO이자 PD로, <V.E.D.A>와 <레벨업 못하는 플레이어>의 개발을 이끌고 있습니다. 네오스트림 인터렉티브에서 <리틀 데빌 인사이드>의 개발 그룹장과 APD를 맡았으며, 웹젠에서는 <SUN>과 <뮤 레전드>의 PD로 활동했습니다. 또한 조이온에서 <천하제일상거상>, <거상 2>, <해상왕 장보고>의 운영 및 기획을 담당했습니다.'),
  ...
  <section class="bg-[#0e0f14] border-b border-[#27272a] pt-24 pb-10">
    <div class="max-w-7xl mx-auto px-6">
      ...
      <div class="flex flex-wrap gap-2 mb-5">
        ...
        <?php foreach (ufs_merged_extra_tracks($session['track'], isset($session['colspan']) ? $session['colspan'] : 1) as $xt): // 통합 세션: 걸치는 트랙 배지 병기 ?>
        <span class="px-1.5 py-1 text-xs rounded-[4px] <?= ufs_track_badge_home($xt) ?>"><?= e(ufs_track_label_day($xt, $session['day'])) ?></span>
        <?php endforeach; ?>
          ...
          <ul class="space-y-2">
            <?php foreach ($session['contents'] as $c):
              ...
              if (preg_match('/^\d+\.\s+/', $ct)):            // "숫자. " → 채운 원(CSS), 레벨1
                ...
                <li class="flex items-start gap-2.5 text-[#a1a1aa]"><span class="flex-shrink-0 mt-[0.55em] w-1 h-1 rounded-full bg-[#00C1D5]"></span><span><?= e($ct) ?></span></li>
              <?php elseif (preg_match('/^[-–·]\s*/u', $ct)):  // "-" → 빈 원/링(CSS), 레벨2(들여쓰기)
                $ct = preg_replace('/^[-–·]\s*/u', '', $ct); ?>
                <li class="flex items-start gap-2.5 text-[#a1a1aa] ml-5"><span class="flex-shrink-0 mt-[0.55em] w-1 h-1 rounded-full border border-[#00C1D5]"></span><span><?= e($ct) ?></span></li>
              <?php else: ?>
                <li class="flex items-start gap-2.5 text-[#a1a1aa]"><span class="flex-shrink-0 mt-[0.55em] w-1 h-1 rounded-full bg-[#00C1D5]"></span><span><?= e($ct) ?></span></li>
              <?php endif; ?>
            <?php endforeach; ?>
