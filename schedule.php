...
require_once __DIR__ . '/_edge_cache.php'; ufs_edge_cache(3600, 60);   // 엣지 캐시(비개인화 공개 페이지) — 프리뷰/관리자/POST 는 자동 제외
...
function ufs_render_track_view($daySessions) {
    foreach (ufs_s_sched_slots($daySessions) as $time) {
        ...
        if ($only_common) {
            ...
            foreach ($inSlot as $s) {
                if (!empty($s['_slot_type']) && $s['_slot_type'] === 'welcome') {
                    ...
                    echo '<div data-sched-common class="h-full flex flex-col justify-center px-6 text-left text-base font-semibold text-white bg-[#0b0c10]"><div>'.e(ufs_slot_common_label($s)).'</div>';
        ...
        foreach ($inSlot as $s) {
            ...
            echo $open.' data-sched-card data-track="'.e($s['track']).'" data-level="'.e($s['level']).'" data-topics="'.e($topics).'" data-products="'.e($prods).'" class="relative block p-6'.$hovcls.'">'.ufs_no_online_ribbon($s['id']);
            $xtb = '';
...
<div class="bg-[#09090b] min-h-screen text-white" data-schedule data-default-view="track"><!-- 기본 뷰 = 리스트(트랙) -->
  ...
  <style>
    ...
    @media (min-width:768px){.ufs-time{width:160px}}
    @media (max-width:639px){
      .ufs-time{width:54px}
      .ufs-time-t{font-size:.72rem;line-height:1.15}
      /* 그리드뷰 시간 셀 축소 */
  ...
  <div class="max-w-7xl mx-auto px-6 pb-24">
    ...
    <p class="text-right text-sm text-[#71717a] mt-6 tracking-tight">· 세션 내용, 발표자 및 일정은 사정에 따라 변경될 수 있습니다.</p>
  </div>
</div>
