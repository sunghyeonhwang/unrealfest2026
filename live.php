...
$TRK  = array(
  '1'=>array('2'=>'게임: 아트','1'=>'게임: 프로그래밍','3'=>'미디어 & 엔터테인먼트','4'=>'공통'),
  '2'=>array('2'=>'게임: 아트','1'=>'게임: 프로그래밍','3'=>'미디어 & 엔터테인먼트','4'=>'제조 및 시뮬레이션'),
);
...
//   서버 정원(2026-08-18 확인): t1→griff-1(1,000) · t2→griff-2(500) · t3→griff-3(1,000) · t4→griff-4(500).
//   day1/day2 는 같은 트랙이면 같은 서버를 쓰고 방 no 만 다르다(d1=1~4, d2=5~8).
//   ⚠️ t2(게임: 아트)는 정원 초과 가능성이 있다. 현장 트랙 분포로 온라인 2,793명을 배분하면
//      t1 915 / t2 575 / t3 915 / t4 387 로, t2 가 정원 500 을 약 75명 넘는다. 게다가 t2 는
//      표시 순서상 첫 채널이라 트랙 미지정 진입까지 흡수한다.
//      1,000짜리 서버가 t1·t3(각 915명)에 이미 필요해 재배정으로는 해결되지 않는다
//      — 어디에 놓아도 한 트랙은 넘친다. 그래서 매핑은 현행 유지.
//      → 2026-08-18 CGChat 에 griff-2 정원 증설 요청함(사무국). 증설되면 코드 수정은 필요 없다
//        (서버 주소는 그대로고 정원만 올라간다). 증설이 안 된 채로 당일을 맞으면 t2 채팅만
//        입장 제한이 걸릴 수 있고, 영상 시청은 채팅과 별개라 영향 없다.
